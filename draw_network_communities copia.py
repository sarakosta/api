...
if __name__ == "__main__":
    ...
    G_nx_controlled, g_gt_controlled, louvain_partition_controlled = \
    ...
    for node_name, comm_id in louvain_partition_controlled.items():
        # Ensure node_name is in the graph-tool map.
        # It's more efficient to access vertices by name if you map them
        v = None
        ...
        # else:
        #     print(f"Warning: Node '{node_name}' from Louvain partition not found in graph-tool graph.")
    ...
    try:
        ...
        # Handle potential division by zero for NMI if one partition has a single community
        ...
        nmi_controlled = mutual_info_score(louvain_labels_controlled, bisbm_labels_controlled) / nmi_denominator_controlled if nmi_denominator_controlled > 0 else 0
        ...
        print(f"Error comparing Louvain partition for Controlled Network with BiSBM: {e}")
    ...
    try:
        ...
        nmi_restored = mutual_info_score(louvain_labels_restored, bisbm_labels_restored) / nmi_denominator_restored if nmi_denominator_restored > 0 else 0
        ...
        print(f"Error comparing Louvain partition for Restored Network with BiSBM: {e}")
