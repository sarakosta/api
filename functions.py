...
def centrality_measures(N_ER, file_path, common_plants, common_pollinators, graph_type):
    ...
    print(f'Weighted Degrees Mann-Withney for {graph_type} network', statistic_mw_wd, p_value_mw_wd)
    
    statistic_ks_wd, p_value_ks_wd = sc.ks_2samp(plant_wd_api, mean_wd_plants)
    print(f'Weighted Degrees Kolmogorov-Smirnov for {graph_type} network', statistic_ks_wd, p_value_ks_wd)
    ...
    print(f'Betweennees Centrality Mann-Withney for {graph_type} network', statistic_mw_bc, p_value_mw_bc)
    
    statistic_ks_bc, p_value_ks_bc = sc.ks_2samp(bc_plants_values_restored, mean_bc_plants)
    print(f'Betweennees Centrality Kolmogorov-Smirnov for {graph_type} network', statistic_ks_bc, p_value_ks_bc)
    ...
    print(f'Closenness Centrality Mann-Withney for {graph_type} network', statistic_mw_bc, p_value_mw_cc)
    
    statistic_ks_cc, p_value_ks_cc = sc.ks_2samp(cc_plants_values_restored, mean_cc_plants)
    print(f'Closeness Centrality Kolmogorov-Smirnov for {graph_type} network', statistic_ks_cc, p_value_ks_cc)
...
def nodes_and_edges(file_adj_matrix,
    ...
    plant_y = compute_vertical_positions(plant_sizes, min_spacing, scale_factor)
    pollinator_y = compute_vertical_positions(pollinator_sizes, min_spacing, scale_factor)
...
def draw_network_order(
    ...
    G, pos, node_sizes, normalized_weights, plant_read, pollinator_read = nodes_and_edges(file_adj_matrix, plant_file, pollinator_file, min_spacing, min_size, scale_factor)
    ...
    legend_patches.append(mpatches.Patch(color="blue", label="Highlighted Plants"))
    legend_patches.append(mpatches.Patch(color="red", label="Other Plants"))

    plt.legend(handles=legend_patches, loc='upper left', bbox_to_anchor=(0.05,0.15), fontsize=30)
    plt.axis('off')
...
def draw_network_communities(
    ...
    plt.legend(handles=legend_handles, loc='upper left', bbox_to_anchor=(0.05,0.15), fontsize=30)    
    plt.axis('off')
...
def print_Louvain_communities(G, N_louvain, network_type, 
    ...
    with open(full_output_txt_filename, 'w', encoding='utf-8') as f:
        ...
        for comm_id, nodes_in_comm in communities_grouped.items():
            ...
            f.write(f"  Plants ({len(plants_in_comm)}): {', '.join(plants_in_comm) if plants_in_comm else 'None'}"+ '\n')
            f.write(f"  Pollinators ({len(pollinators_in_comm)}): {', '.join(pollinators_in_comm) if pollinators_in_comm else 'None'}"+ '\n')
            f.write("-" * 30+ '\n')
...
def print_biSBM_communities(G, N_biSBM, network_type, 
    ...
    bisbm_state = gt.minimize_blockmodel_dl(g_gt, state_args={'bipartite_nodes': g_gt.vp.bipartite})
    optimal_bisbm_dl = bisbm_state.entropy()
    
    for n in range(N_biSBM):
        bisbm_state_new = gt.minimize_blockmodel_dl(g_gt, state_args={'bipartite_nodes': g_gt.vp.bipartite})
    ...
    for v in g_gt.vertices():
        ...
        partition[node_name] = int(community_id) # Ensure integer for consistency

    # Determine the number of communities
    num_communities = bisbm_state.get_B() # In graph-tool SBM, get_B() gives the number of blocks
    ...
    with open(full_output_txt_filename, 'w', encoding='utf-8') as f:
        ...
        f.write(f"Detected {num_communities} communities."+ '\n')
        f.write(f"Optimal Description Length (Entropy): {optimal_bisbm_dl:.4f}"+ '\n')
        f.write("\n--- Community Details ---"+ '\n')
    ...
    return partition, num_communities, optimal_bisbm_dl, communities_grouped, modularity
