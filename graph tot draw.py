...
def print_network(file):
    ...
    row_labels, col_labels, adj_matrix = load_adjacency_matrix(file)
    ...
    normalised_weights = [0.5 + (w / max_weight) * 5 for w in weights]
      
    nx.draw(G, pos, with_labels=True, node_size=200, node_color="lightblue", edge_color="black", font_size=12, width=normalised_weights)
    #nx.draw_networkx_edge_labels(G, pos, edge_labels={(u, v): d['weight'] for u, v, d in edges}, font_size=10)
    
print_network("gcontrolled.csv")
plt.title("Bipartite Pollination Graph Controlled")
    
# Save the figure as PDF
plt.savefig("controlled_graph.pdf", format='pdf', dpi=300, bbox_inches='tight')
plt.show()
...
# Save the figure as PDF
plt.savefig("restored_graph.pdf", format='pdf', dpi=300, bbox_inches='tight')
plt.show()